...
Original file is located at
    https://colab.research.google.com/drive/1MroUAgw3-PidJEfhzaMUY6Hmn4LnFjzZ
"""
...
from sklearn.model_selection import train_test_split as training_testing_split
...
kc_x_train, kc_x_valid, kc_y_train, kc_y_valid = training_testing_split(kc_data.iloc[:,0:12], kc_data.iloc[:,12],
...
history = model.fit(arr_x_train, arr_y_train,
    ...
    validation_data=(arr_x_valid, arr_y_valid),callbacks=[tensorboard])
...
def basic_model_2(x_size, y_size):
    ...
    t_model.add(Dense(20, activation="relu"))
    t_model.add(Dense(y_size))
    keras.optimizers.Adam(lr=0.008, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
    t_model.compile(loss='mean_squared_error',
        optimizer=Adam(),
        metrics=[metrics.mae])
    tensorboard = TensorBoard(log_dir="logs/final1",histogram_freq=0, write_graph=True, write_images=True)
...
def basic_model_2(x_size, y_size):
    ...
    t_model.add(Dropout(0.1))
    t_model.add(Dense(50, activation="softmax"))
    t_model.add(Dense(20, activation="softmax"))
    t_model.add(Dense(y_size))
    keras.optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
    t_model.compile(loss='mean_squared_error',
        optimizer=Adam(),
